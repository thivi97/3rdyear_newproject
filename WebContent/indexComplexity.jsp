<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
 <title>Complexity</title>
 <link rel="icon" type="image/png" href="images/cde.png">
 		<link rel="stylesheet" href="css/navigation.css">
        <link rel="stylesheet" href="css/styles.css">
        <script src="css/letter.js"></script>
        <link rel="stylesheet" href="css/complexity.css">      
        <script src="https://kit.fontawesome.com/b99e675b62.js"></script>
    	<script src="https://cdnjs.cloudflare.com/ajax/libs/animejs/2.0.2/anime.min.js"></script>
    </head>
    <body>
    
   
        <div class="wrapper">
            <div class="sidebar">
                <h2>CDE SOLUTIONS</h2>
                <ul>
                    <li><a href="Home.jsp"><i class="fas fa-home"></i>HOME</a></li>
                    <li><a href="uploadFileSVM.jsp"><i class="fas fa-size"></i>SIZE,METHOD & VARIABLE</a></li>
                    <li><a href="inheritance.jsp"><i class="fas fa-inheritance"></i>INHERITANCE</a></li>
                    <li><a href="coupling.jsp"><i class="fas fa-coupling"></i>COUPLING</a></li>
                    <li><a href="uploadPageControl.jsp"><i class="fas fa-structure"></i>CONTROL STRUCTURE</a></li>
                   
                    <li><a href="login.jsp"><i class="fas fa-total"></i>Logout</a></li>
                </ul>
            </div>
        </div>
        
         
        <br>
        
        <div class="sign">
        	<span class="fast-flicker">COM</span><span>PLEX</span><span class="flicker">ITY</span>
        </div>
        
        <br>
        <img src="css/header.gif" alt="" style="width:400px;height:200px;margin-left :40%;padding:10px;  align:center; margin-top: 100px; ">
        
        <br>
        <div class="point">
        <ul>
        	<li>Complexity due to Size</li>
        	<li>Complexity due to Method</li>
        	<li>Complexity due to Variable</li>
        	<li>Complexity due to Inheritance</li>
        	<li>Complexity due to Coupling</li>
        	<li>Complexity due to ControlStructure</li>
        </ul>
        </div>
        
    </body>
</html>